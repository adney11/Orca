...
if [ ! -d $action_conf_dir ];
then
    mkdir $action_conf_dir
fi

mv "$dir/logs/ood-$act_id.log" "$action_conf_dir/$trace_basename-$trace_id"
mv "$dir/logs/softmax_values_actor-$act_id.log" "$smax_dir/$trace_basename-$trace_id"
#rm "$dir/logs/*"
echo "[$0]: Done"